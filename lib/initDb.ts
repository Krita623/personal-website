import { query, execute, generateUUID } from './mysql';
import { hash } from 'bcrypt';
import { findUserByEmail } from './userUtils';
...
export async function initDatabase(): Promise<boolean> {
  try {
    ...
    try {
      ...
      console.log('[InitDB] 添加索引 idx_userId 成功');
    } catch (err) {
      console.log('[InitDB] 索引 idx_userId 可能已存在');
    }
    
    try {
      await query(`ALTER TABLE solutions ADD INDEX idx_difficulty (difficulty)`);
      console.log('[InitDB] 添加索引 idx_difficulty 成功');
    } catch (err) {
      console.log('[InitDB] 索引 idx_difficulty 可能已存在');
    }
    
    try {
      await query(`ALTER TABLE solutions ADD INDEX idx_category (category)`);
      console.log('[InitDB] 添加索引 idx_category 成功');
    } catch (err) {
      console.log('[InitDB] 索引 idx_category 可能已存在');
    }
    
    try {
      await query(`ALTER TABLE solutions ADD INDEX idx_updatedAt (updatedAt)`);
      console.log('[InitDB] 添加索引 idx_updatedAt 成功');
    } catch (err) {
      console.log('[InitDB] 索引 idx_updatedAt 可能已存在');
    }
    
    try {
      await query(`ALTER TABLE messages ADD INDEX idx_messages_isRead (isRead)`);
      console.log('[InitDB] 添加索引 idx_messages_isRead 成功');
    } catch (err) {
      console.log('[InitDB] 索引 idx_messages_isRead 可能已存在');
    }
    ...
    console.error('[InitDB] 数据库初始化失败:', error);
    return false;
  }
}
 ...
 * 确保存在管理员用户
 */
export async function ensureAdminUser(email: string, password: string, name: string): Promise<void> {
  try {
    console.log(`[InitDB] 检查管理员用户: ${email}`);
    ...
    console.error('[InitDB] 创建管理员用户失败:', error);
    throw error;
  }
}
